...
✅ **Git规范管理**：双分支开发规范，API Key安全隔离，符合工业级开发流程
...
- Python 3.10 及以上版本
- 有效的 DeepSeek API Key（可从 [DeepSeek开放平台](https://platform.deepseek.com) 获取）
...
打开 config.yaml，填入你的 DeepSeek API Key，按需调整其他配置项。

### 4. 放入待解析文档

将需要解析的硬件数据手册 / 通信协议文档，放入 input_docs 目录。
...
cp config.yaml.example config.yaml
打开 config.yaml，填入你的 DeepSeek API Key，按需调整其他配置项。4. 放入待解析文档
将需要解析的硬件数据手册 / 通信协议文档，放入 input_docs 目录。5. 运行使用
基础使用（使用配置文件默认文档）：
bash
运行
python hw_datasheet_agent.py
...
python hw_datasheet_agent.py -f FS5708-PALM.pdf -o FS5708掌静脉模组通信协议归档 6. 查看结果
生成的归档文档会自动保存在 output_archive 目录下，按时间戳创建子文件夹，包含 Markdown/Word/Excel 三种格式的文件。
项目目录结构
text
hardware_datasheet_read_agent/
├── input_docs/ # 待解析的硬件文档存放目录（目录结构提交，内部文档不上传Git）
├── output_archive/ # 自动生成：归档文档输出目录
├── agent_logs/ # 自动生成：程序运行日志目录
├── .gitignore # Git忽略规则配置
├── config.yaml # 私密配置文件（不上传Git）
├── config.yaml.example # 配置文件模板（可上传Git）
